...
// GetColor takes in a syntax group and returns the theme's style for that group
func GetColor(color string) tcell.Style {
	return theme.GetColor(color)
}

// GetColor takes in a syntax group and returns the theme's style for that group
func (theme Theme) GetColor(color string) tcell.Style {
	st := defStyle
	if color == "" {
		return st
	}
	...
	if len(groups) > 1 {
		...
		for i, g := range groups {
			...
			curGroup += g
			if style, ok := theme[curGroup]; ok {
				st = style
			}
		}
	} else if style, ok := theme[color]; ok {
		st = style
	} else {
...
// init picks and initializes the theme when micro starts
func init() {
	theme = make(Theme)
...
// SetDefaultTheme sets the current default theme for new Views.
func SetDefaultTheme(scheme Theme) {
	theme = scheme
}

// ParseTheme parses the text definition for a theme and returns the corresponding object
// Themes are made up of color-link statements linking a color group to a list of colors
// For example, color-link keyword (blue,red) makes all keywords have a blue foreground and
// red background
func ParseTheme(text string) Theme {
