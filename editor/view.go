...
// SetTheme sets the theme for this view.
func (v *View) SetTheme(name string) bool {
	for _, theme := range Assets.Themes {
		if theme.Name == name {
			v.theme = ParseTheme(string(theme.Data))
			v.Buf.updateRules()
			return true
		}
	}
...
func (v *View) displayView(screen tcell.Screen) {
	...
	v.cellview.Draw(v.Buf, v.theme, top, height, left, width-v.lineNumOffset)
	...
	for visualLineN, line = range v.cellview.lines {
		...
		if colorcolumn != 0 && xOffset+colorcolumn-v.leftCol < v.width {
			style := v.theme.GetColor("color-column")
		...
		if v.Buf.Settings["ruler"] == true {
			...
			if style, ok := v.theme["line-number"]; ok {
				lineNumStyle = style
			}
			if style, ok := v.theme["current-line-number"]; ok {
			...
			if char != nil {
				...
				for _, c := range v.Buf.cursors {
					...
					if v.Cursor.HasSelection() &&
						...
						if style, ok := v.theme["selection"]; ok {
							lineStyle = style
						}
					}
				}
				...
				if v.Buf.Settings["cursorline"].(bool) &&
					...
					style := v.theme.GetColor("cursor-line")
					fg, _, _ := style.Decompose()
					lineStyle = lineStyle.Background(fg)
				}
