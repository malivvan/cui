...
func escapeSelector(s string) string { return specialCharReplacer.Replace(s) }

func (ts tagSelector) String() string {
	return ts.tag
}

func (is idSelector) String() string {
	return "#" + escapeSelector(is.id)
}

func (cs classSelector) String() string {
	return "." + escapeSelector(cs.class)
}

func (as attrSelector) String() string {
	val := as.val
	if as.operation == "#=" {
		val = as.regexp.String()
	...
	return fmt.Sprintf(`[%s%s%s%s]`, as.key, as.operation, val, ignoreCase)
}
