...
func TestVariable(t *testing.T) {
	vars := []VariableTest{
		...
		},
		{
			...
			StringValue: "52.11",
			BoolValue:   true,
			Object:      &runtime.String{Value: "52.11"},
		},
		{
			Name:        "c",
			Value:       true,
			...
			BoolValue:   true,
			StringValue: "true",
			Object:      runtime.TrueValue,
		},
		{
			Name:        "d",
			Value:       nil,
			ValueType:   "undefined",
			Object:      runtime.UndefinedValue,
			IsUndefined: true,
		},
	}

	for _, tc := range vars {
		v, err := markup.NewVariable(tc.Name, tc.Value)
