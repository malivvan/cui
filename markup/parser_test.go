...
func dumpLevel(w io.Writer, n *Node, level int) error {
	...
	case ElementNode:
		if n.NS != "" {
			fmt.Fprintf(w, "<%s %s>", n.NS, n.Tag)
		} else {
			fmt.Fprintf(w, "<%s>", n.Tag)
		}
		attr := sortedAttributes(n.Attrs)
		sort.Sort(attr)
		for _, a := range attr {
			io.WriteString(w, "\n")
			dumpIndent(w, level)
...
var renderTestBlacklist = map[string]bool{
	...
	// A <plaintext> element can't have anything after it in
...
func TestNodeConsistency(t *testing.T) {
	// inconsistentNode is a Node whose Atom and Tag do not agree.
