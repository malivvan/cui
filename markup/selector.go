...
type Sel interface {
	...
	// String returns a CSS input compiling to this selector.
	String() string

	// PseudoElement returns a pseudo-element, or an empty string.
	PseudoElement() string
}

// ParseSelector parses a selector. Use `ParseSelectorWithPseudoElement`
// if you need support for pseudo-elements.
func ParseSelector(sel string) (Sel, error) {
	p := &selectorParser{s: sel}
...
// ParseSelectorWithPseudoElement parses a single selector,
// with support for pseudo-element.
func ParseSelectorWithPseudoElement(sel string) (Sel, error) {
	p := &selectorParser{s: sel, acceptPseudoElements: true}
	compiled, err := p.parseSelector()
	if err != nil {
		return nil, err
	}
...
// ParseSelectorGroup parses a selector, or a group of selectors separated by commas.
// Use `ParseSelectorGroupWithPseudoElements`
// if you need support for pseudo-elements.
func ParseSelectorGroup(sel string) (SelectorGroup, error) {
	p := &selectorParser{s: sel}
...
// ParseSelectorGroupWithPseudoElements parses a selector, or a group of selectors separated by commas.
// It supports pseudo-elements.
func ParseSelectorGroupWithPseudoElements(sel string) (SelectorGroup, error) {
	p := &selectorParser{s: sel, acceptPseudoElements: true}
	compiled, err := p.parseSelectorGroup()
	if err != nil {
		return nil, err
...
// CompileSelector parses a selector and returns, if successful, a Selector object
// that can be used to match against Node objects.
func CompileSelector(sel string) (Selector, error) {
	compiled, err := ParseSelectorGroup(sel)
	if err != nil {
		return nil, err
	}
...
// MustCompileSelector is like CompileSelector, but panics instead of returning an error.
func MustCompileSelector(sel string) Selector {
...
func queryInto(n *Node, m Matcher, storage []*Node) []*Node {
	for child := n.FirstChild; child != nil; child = child.NextSibling {
		if m.Match(child) {
...
// of n.
func QueryAll(n *Node, m Matcher) []*Node {
...
func Query(n *Node, m Matcher) *Node {
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		if m.Match(c) {
			return c
		}
...
func Filter(nodes []*Node, m Matcher) (result []*Node) {
	for _, n := range nodes {
		if m.Match(n) {
			result = append(result, n)
		}
...
func (ts tagSelector) Match(n *Node) bool {
	return n.Type == ElementNode && n.Tag == ts.tag
}

func (ts tagSelector) Specificity() Specificity {
	return Specificity{0, 0, 1}
}

func (ts tagSelector) PseudoElement() string {
	return ""
}
...
// Match matches elements by class attribute.
func (cs classSelector) Match(n *Node) bool {
	return matchAttribute(n, "class", func(s string) bool {
		return matchInclude(cs.class, s, false)
	})
}

func (cs classSelector) Specificity() Specificity {
	return Specificity{0, 1, 0}
}

func (cs classSelector) PseudoElement() string {
	return ""
}
...
// Match matches elements by id attribute.
func (is idSelector) Match(n *Node) bool {
	return matchAttribute(n, "id", func(s string) bool {
		return s == is.id
	})
}

func (is idSelector) Specificity() Specificity {
	return Specificity{1, 0, 0}
}

func (is idSelector) PseudoElement() string {
	return ""
}
...
// Match matches elements by attribute value.
func (as attrSelector) Match(n *Node) bool {
	switch as.operation {
	case "":
		return matchAttribute(n, as.key, func(string) bool { return true })
	case "=":
		return matchAttribute(n, as.key, func(s string) bool { return matchInsensitiveValue(s, as.val, as.insensitive) })
	case "!=":
		return attributeNotEqualMatch(as.key, as.val, n, as.insensitive)
	case "~=":
		// matches elements where the attribute named key is a whitespace-separated list that includes val.
		return matchAttribute(n, as.key, func(s string) bool { return matchInclude(as.val, s, as.insensitive) })
	case "|=":
		return attributeDashMatch(as.key, as.val, n, as.insensitive)
	case "^=":
		return attributePrefixMatch(as.key, as.val, n, as.insensitive)
	case "$=":
		return attributeSuffixMatch(as.key, as.val, n, as.insensitive)
	case "*=":
		return attributeSubstringMatch(as.key, as.val, n, as.insensitive)
	case "#=":
		return attributeRegexMatch(as.key, as.regexp, n)
	default:
		panic(fmt.Sprintf("unsuported operation : %s", as.operation))
	}
}
...
func matchAttribute(n *Node, key string, f func(string) bool) bool {
...
func attributeNotEqualMatch(key, val string, n *Node, ignoreCase bool) bool {
	if n.Type != ElementNode {
		return false
	}
	for _, a := range n.Attrs {
...
func attributeDashMatch(key, val string, n *Node, ignoreCase bool) bool {
...
func attributePrefixMatch(key, val string, n *Node, ignoreCase bool) bool {
	return matchAttribute(n, key,
		func(s string) bool {
...
func attributeSuffixMatch(key, val string, n *Node, ignoreCase bool) bool {
	return matchAttribute(n, key,
		func(s string) bool {
			if strings.TrimSpace(s) == "" {
				return false
...
func attributeSubstringMatch(key, val string, n *Node, ignoreCase bool) bool {
	return matchAttribute(n, key,
		func(s string) bool {
			if strings.TrimSpace(s) == "" {
				return false
...
func attributeRegexMatch(key string, rx *regexp.Regexp, n *Node) bool {
	return matchAttribute(n, key,
		func(s string) bool {
...
func (as attrSelector) Specificity() Specificity {
	return Specificity{0, 1, 0}
}

func (as attrSelector) PseudoElement() string {
	return ""
}
...
func (c neverMatchSelector) Match(n *Node) bool {
	return false
}

func (c neverMatchSelector) Specificity() Specificity {
...
func (c compoundSelector) Match(n *Node) bool {
...
func (c combinedSelector) Match(n *Node) bool {
	if c.first == nil {
		return false // maybe we should panic
	}
	switch c.combinator {
	case 0:
		return c.first.Match(n)
	case ' ':
		return descendantMatch(c.first, c.second, n)
	case '>':
		return childMatch(c.first, c.second, n)
	case '+':
		return siblingMatch(c.first, c.second, true, n)
	case '~':
		return siblingMatch(c.first, c.second, false, n)
	default:
...
func descendantMatch(a, d Matcher, n *Node) bool {
	if !d.Match(n) {
		return false
	}
...
func childMatch(a, d Matcher, n *Node) bool {
...
func siblingMatch(s1, s2 Matcher, adjacent bool, n *Node) bool {
	if !s2.Match(n) {
		return false
	}
...
func (c combinedSelector) Specificity() Specificity {
	spec := c.first.Specificity()
	if c.second != nil {
		spec = spec.Add(c.second.Specificity())
	}
	return spec
}
...
func (c SelectorGroup) Match(n *Node) bool {
	for _, sel := range c {
		if sel.Match(n) {
			return true
		}
...
// a selectorParser for CSS selectors
...
func (p *selectorParser) parseEscape() (result string, err error) {
...
func (p *selectorParser) parseIdentifier() (result string, err error) {
	const prefix = '-'
	var numPrefix int
...
func (p *selectorParser) parseName() (result string, err error) {
	i := p.i
loop:
	for i < len(p.s) {
		c := p.s[i]
...
func (p *selectorParser) parseString() (result string, err error) {
	i := p.i
	if len(p.s) < i+2 {
...
func (p *selectorParser) parseRegex() (rx *regexp.Regexp, err error) {
	i := p.i
	if len(p.s) < i+2 {
...
func (p *selectorParser) consumeParenthesis() bool {
...
// whitespace. It returns true if there was actually a parenthesis to skip.
func (p *selectorParser) consumeClosingParenthesis() bool {
	i := p.i
	p.skipWhitespace()
	if p.i < len(p.s) && p.s[p.i] == ')' {
		p.i++
...
func (p *selectorParser) parseTypeSelector() (result tagSelector, err error) {
...
func (p *selectorParser) parseIDSelector() (idSelector, error) {
	if p.i >= len(p.s) {
...
func (p *selectorParser) parseClassSelector() (classSelector, error) {
	if p.i >= len(p.s) {
...
func (p *selectorParser) parseAttributeSelector() (attrSelector, error) {
	if p.i >= len(p.s) {
...
func (p *selectorParser) parsePseudoclassSelector() (out Sel, pseudoElement string, err error) {
	if p.i >= len(p.s) {
...
func (p *selectorParser) parseInteger() (int, error) {
	i := p.i
	start := i
...
// a single element.
func (p *selectorParser) parseSimpleSelectorSequence() (Sel, error) {
	var selectors []Sel
...
func (p *selectorParser) parseSelector() (Sel, error) {
	p.skipWhitespace()
...
func (p *selectorParser) parseSelectorGroup() (SelectorGroup, error) {
