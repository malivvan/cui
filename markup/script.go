...
import (
	...
	"github.com/malivvan/cui/runtime/encoding"
...
func (s *Script) Add(name string, value interface{}) error {
	obj, err := runtime.FromInterface(value)
	if err != nil {
		return err
	}
...
func (s *Script) SetImports(modules *runtime.ModuleMap) {
	s.modules = modules
}
...
func (s *Script) Compile() (*Program, error) {
	...
	c := runtime.NewCompiler(srcFile, symbolTable, nil, s.modules, nil)
	...
	for _, name := range symbolTable.Names() {
		...
		if symbol.Scope == runtime.ScopeGlobal {
...
func (s *Script) prepCompile() (symbolTable *runtime.SymbolTable, globals []runtime.Object, err error) {
	var names []string
	for name := range s.variables {
		names = append(names, name)
	}

	symbolTable = runtime.NewSymbolTable()
...
type Program struct {
	...
	globals       []runtime.Object
	maxAllocs     int64
	lock          sync.RWMutex
}

// Bytecode returns the compiled bytecode of the Program.
func (p *Program) Bytecode() *runtime.Bytecode {
	p.lock.RLock()
...
func (p *Program) Unmarshal(b []byte) (err error) {
	...
	n, p.globals, err = encoding.UnmarshalSlice[runtime.Object](n, body, runtime.UnmarshalObject)
	if err != nil {
		return err
	}
...
func (p *Program) Marshal() ([]byte, error) {
	...
	data := make([]byte,
		encoding.SizeMap[string, int](p.globalIndices, encoding.SizeString, encoding.SizeInt)+
			encoding.SizeSlice[runtime.Object](p.globals, runtime.SizeOfObject)+
			encoding.SizeInt64())
	n = encoding.MarshalMap[string, int](n, data, p.globalIndices, encoding.MarshalString, encoding.MarshalInt)
	n = encoding.MarshalSlice[runtime.Object](n, data, p.globals, runtime.MarshalObject)
...
func (p *Program) Run() error {
	...
	v := runtime.NewVM(context.Background(), p.bytecode, p.globals, p.maxAllocs)
	return v.Run()
}
...
func (p *Program) RunContext(ctx context.Context) (err error) {
	...
	v := runtime.NewVM(ctx, p.bytecode, p.globals, p.maxAllocs)
...
func (p *Program) Clone() *Program {
	...
	clone := &Program{
		...
		bytecode:      p.bytecode,
		globals:       make([]runtime.Object, len(p.globals)),
...
func (p *Program) IsDefined(name string) bool {
	...
	return v != runtime.UndefinedValue
}
