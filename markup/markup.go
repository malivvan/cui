...
import (
	...
	"github.com/malivvan/cui/markup/sh"
...
func RunREPL(ctx context.Context, in io.Reader, out io.Writer, prompt string) {
	...
	globals[symbol.Index] = &runtime.BuiltinFunction{
		Name: "println",
		Value: func(ctx context.Context, args ...runtime.Object) (ret runtime.Object, err error) {
			var printArgs []interface{}
			for _, arg := range args {
				if _, isUndefined := arg.(*runtime.Undefined); isUndefined {
	...
	for {
		...
		file = addPrints(file)
		c := runtime.NewCompiler(srcFile, symbolTable, constants, Modules, nil)
		...
		bytecode := c.Bytecode()
		machine := runtime.NewVM(ctx, bytecode, globals, -1)
