...
func (c callres) call(funcName string, args ...interface{}) callres {
	...
	var oargs []runtime.Object
	for _, v := range args {
		oargs = append(oargs, object(v))
	}

	switch o := c.o.(type) {
	case *runtime.BuiltinModule:
		...
		f, ok := m.(*runtime.BuiltinFunction)
		if !ok {
			return callres{t: c.t, e: fmt.Errorf(
...
func object(v interface{}) runtime.Object {
	switch v := v.(type) {
	case runtime.Object:
		return v
	case string:
	...
	case time.Time:
		return &runtime.Time{Value: v}
	case []int:
		var objs []runtime.Object
		for _, e := range v {
			objs = append(objs, &runtime.Int{Value: int64(e)})
		}
