...
func TestTimes(t *testing.T) {
	...
	if _runtime.GOOS == "windows" {
		t.Skipf("skipping test on %s", _runtime.GOOS)
	}
	...
	// TODO: maybe
	// module(t, "times").call("sleep", 1).expect(runtime.UndefinedValue)

	require.True(t, module(t, "times").
		call("since", time.Now().Add(-time.Hour)).
		o.(*runtime.Int).Value > 3600000000000)
	require.True(t, module(t, "times").
	...
	nowD := time.Until(module(t, "times").call("now").
		o.(*runtime.Time).Value).Nanoseconds()
