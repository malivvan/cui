...
const (
	...
	// Block: Block Elements U+2580-U+259F (http://unicode.org/charts/PDF/U2580.pdf)
...
func drawLine(screen tcell.Screen, startX int, startY int, length int, mode drawLineMode, color tcell.Color) {
	if mode == horizontalLine {
		for i := range length {
			PrintJoinedSemigraphics(screen, startX+i, startY, BoxDrawingsLightTripleDashHorizontal, color)
		}
	} else if mode == verticalLine {
		for i := range length {
			PrintJoinedSemigraphics(screen, startX, startY+i, BoxDrawingsLightTripleDashVertical, color)
		}
	}
}
