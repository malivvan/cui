...
func drawLine(screen tcell.Screen, startX int, startY int, length int, mode drawLineMode, color tcell.Color) {
	if mode == horizontalLine {
		for i := range length {
			cui.PrintJoinedSemigraphics(screen, startX+i, startY, cui.BoxDrawingsLightTripleDashHorizontal, color)
		}
	} else if mode == verticalLine {
		for i := range length {
			cui.PrintJoinedSemigraphics(screen, startX, startY+i, cui.BoxDrawingsLightTripleDashVertical, color)
		}
	}
}
