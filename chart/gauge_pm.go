...
func (g *PercentageModeGauge) Draw(screen tcell.Screen) {
	...
	style := tcell.StyleDefault.Background(g.pgBgColor).Foreground(cui.Styles.PrimaryTextColor)
	...
	for j := range pcRune {
		style = tcell.StyleDefault.Background(cui.Styles.PrimitiveBackgroundColor).Foreground(cui.Styles.PrimaryTextColor)
		if x+prgBlock >= tX+j {
			style = tcell.StyleDefault.Background(g.pgBgColor).Foreground(cui.Styles.PrimaryTextColor)
		}
