...
func NewMessageDialog(dtype int) *MessageDialog {
	...
	dialog.textview = cui.NewTextView()
	dialog.textview.SetDynamicColors(true)
	dialog.textview.SetWrap(true)
	dialog.textview.SetTextAlign(cui.AlignLeft)

	dialog.form = cui.NewForm()
	dialog.form.AddButton("Enter", nil)
	dialog.form.SetButtonsAlign(cui.AlignRight)

	dialog.layout = cui.NewFlex()
	dialog.layout.SetDirection(cui.FlexRow)
