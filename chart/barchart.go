...
func (c *BarChart) Draw(screen tcell.Screen) { //nolint:funlen,cyclop
	...
	cui.PrintJoinedSemigraphics(screen,
		x+maxValLenght,
		xAxisStartY,
		cui.BoxDrawingsLightUpAndRight, c.axesColor)

	cui.PrintJoinedSemigraphics(screen, x+maxValLenght-1, xAxisStartY, '0', c.axesLabelColor)

	mxValRune := []rune(maxValueSr)
	for i := range mxValRune {
		cui.PrintJoinedSemigraphics(screen, x+borderPadding+i, maxValY, mxValRune[i], c.axesLabelColor)
	}
	...
	for _, item := range c.bars {
		...
		r := []rune(item.label)
		for j := range r {
			cui.PrintJoinedSemigraphics(screen, startX+j, labelY, r[j], item.color)
		}
		// bar style
		...
		for k := range barHeight {
			for l := range c.barWidth {
				cui.PrintJoinedSemigraphics(screen, startX+l, barStartY-k, fullBlockRune, item.color)
			}
		}
		// bar value
		...
		for i := range vRune {
			cui.PrintJoinedSemigraphics(screen, startX+i, barStartY-barHeight, vRune[i], item.color)
		}
