	...
	for _, c := range textViewTestCases {
		...
		t.Run(c.String(), func(t *testing.T) {
			...
			if c.app {
				...
				if err != nil {
					t.Errorf("failed to prepare append Text: %s", err)
				}
	...
	for _, c := range textViewTestCases {
		...
		b.Run(c.String(), func(b *testing.B) {
			...
			if c.app {
				...
				if err != nil {
					b.Errorf("failed to prepare append Text: %s", err)
				}
			}
...
func TestTextViewMaxLines(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if err != nil {
			t.Errorf("failed to write to Text: %s", err)
		}
	}
...
func tvc(c *textViewTestCase) *Text {
	tv := NewTextView()
...
func prepareAppendTextView(t *Text) ([]byte, error) {
	var b []byte
