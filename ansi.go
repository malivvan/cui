...
// ansi is an io.Writer which translates ANSI escape codes into color tags.
type ansi struct {
	io.Writer
...
// written to it into color tags. Other escape codes don't have an effect
		...
		case ansiControlSequence:
			...
			case r >= 0x40 && r <= 0x7e: // Final byte.
				...
				case 'E': // Next line.
					...
					_, _ = fmt.Fprint(a.buffer, strings.Repeat("\n", count))
				...
				FieldLoop:
					...
					if len(foreground) > 0 || len(background) > 0 || len(a.attributes) > 0 {
						_, _ = fmt.Fprintf(a.buffer, "[%s:%s%s%s]", foreground, background, colon, a.attributes)
					}
				}
				a.state = ansiText
