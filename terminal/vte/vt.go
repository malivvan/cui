...
func (vt *VT) Draw() {
	...
	for row := 0; row < vt.height(); row += 1 {
		for col := 0; col < vt.width(); {
			...
			w := cell.width
			vt.surface.SetContent(col, row, cell.content, cell.combining, cell.attrs.Background(tcell.NewRGBColor(0, 0, 0)))
			if w == 0 {
				w = 1
			}
			col += w
	...
	// 	// DECSIXEL Introducer(\033P0;0;8q) + DECGRA ("1;1): Set Raster Attrs
	// 	os.Stdout.Write([]byte{0x1b, 0x50, 0x30, 0x3b, 0x30, 0x3b, 0x38, 0x71, 0x22, 0x31, 0x3b, 0x31})
	// 	os.Stdout.Write(s.Sixel.Tag)
