...
func visualToCharPos(visualIndex int, lineN int, str string, buf *Buffer, theme Theme, tabsize int) (int, int, *tcell.Style) {
	charPos := 0
	var lineIdx int
	var lastWidth int
...
func (c *CellView) Draw(buf *Buffer, theme Theme, top, height, left, width int) {
	if width <= 0 {
		return
	}
	...
	for viewLine < height {
		...
		colN, startOffset, startStyle := visualToCharPos(left, lineN, lineStr, buf, theme, tabsize)
		if colN < 0 {
			colN = len(line)
		}
		viewCol := -startOffset
		...
		for viewCol < lineLength {
			...
			if group, ok := buf.Match(lineN)[colN]; ok {
				curStyle = theme.GetColor(group.String())
			}
			...
			if char == '\t' {
				...
				if viewCol >= 0 {
					...
					if group, ok := theme["indent-char"]; ok && !IsStrWhitespace(ch) && ch != "" {
						indentStyle = group
					}
